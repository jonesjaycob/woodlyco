...
import { getInventoryById, getInventory } from "@/lib/actions/inventory";
...
export async function generateMetadata({
  ...
  const priceDisplay = (item.price / 100).toLocaleString();

  return {
    title: item.name,
    description: `${item.description} ${item.wood} light post with ${powerLabels[item.power].toLowerCase()}. $${priceDisplay}.`,
    openGraph: {
                  ...
                  <div className="p-4">
                    ...
                    {relatedItem.status === "available" && (
                      <p className="text-lg font-bold">
                        ${(relatedItem.price / 100).toLocaleString()}
                      </p>
                    )}
                  </div>
                </Card>
